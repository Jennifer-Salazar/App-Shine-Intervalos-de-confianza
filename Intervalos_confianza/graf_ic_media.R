...
graf_pivote_media <- function(x_barra, desv, li, ls, n, conocida, normalidad){
  ...
  if(normalidad & !conocida){
    ...
    Z <- rnorm(10000)
    Chi2 <- rchisq(10000, v)
    T_student <- (Z + x_barra)/sqrt(Chi2/v)
    
    # Se simula el eje x y el eje y
    eje_x <- density(T_student)$x
    eje_y <- density(T_student)$y
    ...
    if(x_barra > 0){
      rango <- c(min(T_student), x_barra+3*sd(T_student))
    }else if(x_barra == 0){
      rango <- c(x_barra-3*sd(T_student), x_barra+3*sd(T_student))
    }else{
...
graf_mv_media <- function(x_barra, desv, n, conocida, li, ls){
  ...
  plot(eje_x, eje_y, ylab="", xlab="", type="l", lwd=2, bty = "n", xaxt="n", xlim= c(x_barra-3*desv, x_barra+3*desv))
  polygon(x_ic, y_ic, col = rgb(1, 0, 0, alpha = 0.5))
  abline(v=x_barra, lty=3)
  axis(1, at = round(c(li, x_barra, ls), 3))
